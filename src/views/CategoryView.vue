<template>
  ...
  <v-data-table :items="items" :headers="headers">
    <template #item.photo="{ item }">
      <v-img :src="item.photo" width="100" height="100" :alt="item.name"></v-img>
    </template>
      ...
      <v-card-text>
        <v-form @submit.prevent="submitForm" ref="form">
          <v-row>
            ...
            <v-col cols="12">
              <v-text-field v-model="formData.photo" label="Photo URL"></v-text-field>
            </v-col>
          </v-row>
        </v-form>
      </v-card-text>
...
const openFormDialog = (item = null) => {
  ...
  if (item) {
    formData.value = { name: item?.name, slug: item?.slug, description: item?.description, photo: item?.photo }
    isEdit.value = true
