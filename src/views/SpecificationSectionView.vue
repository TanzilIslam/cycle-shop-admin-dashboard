...
const moduleName = 'Specification Section'
...
const items = computed(() => specificationSectionStore.specificationSections)
...
async function confirmDelete(item) {
  ...
  if (confirmed) {
    try {
      await specificationSectionStore.remove(item.id)
...
const submitForm = async () => {
  ...
  try {
    if (isEdit.value) {
      await specificationSectionStore.update(resourceId.value, formData.value)
    } else {
      await specificationSectionStore.create(formData.value)
    }
    appStore.showSnackbar({
